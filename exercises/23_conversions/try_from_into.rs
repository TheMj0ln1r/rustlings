...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let array: Vec<u8> = arr.iter().filter(|&item| (0..=255).contains(item)).map(|&item| item as u8).collect();
        if array.len() != 3 {
            Err(IntoColorError::IntConversion)
        }
        else {
            Ok(Self { red: array[0], green: array[1], blue: array[2] })
        }
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        let array: Vec<u8> = slice.iter().filter(|&item| (0..=255).contains(item)).map(|&item| item as u8).collect();
        if array.len() != 3 {
            Err(IntoColorError::IntConversion)
        }
        else {
